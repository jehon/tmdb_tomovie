...
const key = fs.readFileSync(process.env.HOME + '//restricted/tmdb.key', 'utf-8');
...
function apiCall(url, args) {
    const fullUrl = 'https://api.themoviedb.org' + url;
...
export default class ScraperTMDB {
    ...
    async search(name, year) {
        return apiCall(`/3/search/movie?language=${this.#lang}&include_adult=false&query=${name}&year=${year}`)
